...
describe('HistoryControls Flow', () => {
  beforeEach(() => {
    cleanup()
  })

  it('should allow navigating through history with undo and redo buttons', () => {
    const jumpTo = vi.fn()
    const history = [[], [], [], []]
    let currentMove = 3
    ...
    const updatedUndoButton = screen.getByRole('button', { name: /undo/i })
    const updatedRedoButton = screen.getByRole('button', { name: /redo/i })

    expect(updatedUndoButton.disabled).toBe(false)
    expect(updatedRedoButton.disabled).toBe(false)

    fireEvent.click(updatedRedoButton)

    expect(jumpTo).toHaveBeenCalledWith({ to: 'next' })
    ...
    const thirdRedoButton = screen.getByRole('button', { name: /redo/i })

    expect(thirdRedoButton.disabled).toBe(true)
  })
})
